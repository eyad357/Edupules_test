...
class Department(Base):
    ...
    # Schema: departments.id is BIGSERIAL — must use BigInteger, not Integer
    id                = Column(BigInteger, primary_key=True, index=True)
    name              = Column(String(150), unique=True, nullable=False)
    code              = Column(String(20), unique=True, nullable=False)
    # Schema: fk_dept_head REFERENCES professors(id) ON DELETE SET NULL
    head_professor_id = Column(BigInteger, ForeignKey("professors.id", ondelete="SET NULL"), nullable=True)
...
class ActivityLog(Base):
    ...
    id               = Column(Integer, primary_key=True, index=True)
    # Schema: ON DELETE SET NULL — student_id is nullable
    student_id       = Column(Integer, ForeignKey("students.id", ondelete="SET NULL"), nullable=True)
    action           = Column(String(100))
    duration_minutes = Column(Integer)
    # Schema: metadata_json is JSONB, not Text
...
class RiskAssessment(Base):
    ...
    id                          = Column(Integer, primary_key=True, index=True)
    # Schema: ON DELETE RESTRICT — academic record must be preserved
...
class InterventionPlan(Base):
    ...
    id          = Column(Integer, primary_key=True, index=True)
    # Schema: ON DELETE RESTRICT — intervention records must be preserved
    student_id  = Column(Integer, ForeignKey("students.id", ondelete="RESTRICT"))
...
class Quiz(Base):
    ...
    id               = Column(Integer, primary_key=True, index=True)
    title            = Column(String(255))
    # Schema: ON DELETE RESTRICT — cannot delete course with quizzes
    course_id        = Column(Integer, ForeignKey("courses.id", ondelete="RESTRICT"))
    # Schema: ON DELETE SET NULL — quiz survives user deletion
...
class QuizSubmission(Base):
    ...
    id             = Column(Integer, primary_key=True, index=True)
    # Schema: ON DELETE RESTRICT — submissions are permanent academic records
    quiz_id        = Column(Integer, ForeignKey("quizzes.id", ondelete="RESTRICT"))
    student_id     = Column(Integer, ForeignKey("students.id", ondelete="RESTRICT"))
